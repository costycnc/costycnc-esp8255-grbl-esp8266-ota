...
/*
  <form action="http://192.168.4.1/update"method="post" enctype="multipart/form-data">
    <input type="file" name="name">
    <input class="button" type="submit" value="Upload">
</form>
*/
...
const char* serverIndex = "REFIRMWARE:<br><form method='POST' action='/update' enctype='multipart/form-data'><input type='file' name='update'><input type='submit' value='Update'></form>";
...
readparam();
  ...
  server.on("/update", HTTP_GET, []() {                 // if the client requests the upload page
       server.sendHeader("Connection", "close");
   ...
   server.on("/update", HTTP_POST,                       // if the client posts to the upload page
    [](){ server.send(200, "text/plain", (Update.hasError()) ? "FAIL" : "OK");
      ESP.restart(); },                          // Send status 200 (OK) to tell the client we are ready to receive
    handleFileUpdate                                    // Receive and save the file
  );
